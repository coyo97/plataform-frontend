      ...
      startPublisherMedia()
        .then((stream) => {
          ...
          signaling.onRequestOffer(({ viewerSocketId }) => {
            ...
            // Añadir/asegurar ambas tracks (audio y video) del streamer
            const need = { audio: true, video: true };
            pcDir.getSenders().forEach(s => {
            ...
            if (!ls) {
              ...
              if (need.audio) {
                ...
                if (at) {
                  ...
                  console.warn('[[CLT-PC]] no audio track en localStreamRef');
                }
              }
              if (need.video) {
                ...
                if (vt) {
                  ...
                  console.warn('[[CLT-PC]] no video track en localStreamRef');
                }
              }
            }

            // Crear/renovar la oferta dirigida
