...
const loadCareers = async () => {
  ...
  setCareers(careers || []);
	...
	const clear = () => {
		setSel(null);
		setName(''); setCode(''); setCareerIds([]);
		setLevel(''); setCredits(''); setIsFundamental(false);
	};
	...
	const save = async () => {
		...
		// Solo agrega la bandera si existe en tu backend
		if (typeof isFundamental === 'boolean') payload.isFundamental = isFundamental;
	...
	const careersMap = useMemo(() => new Map(careers.map(c => [c._id, c.name])), [careers]);
	...
	return (
		<Container>
			...
			<Input type="number" placeholder="Nivel/semestre (opcional)" value={level} onChange={e => setLevel(e.target.value === '' ? '' : Number(e.target.value))} />
			<Input type="number" placeholder="Créditos (opcional)" value={credits} onChange={e => setCredits(e.target.value === '' ? '' : Number(e.target.value))} />

			{/* Checkbox “fundamental” (si decides usarlo en BE) */}
			...
			<Button onClick={save}>{sel ? 'Actualizar' : 'Crear'} materia</Button>
					...
					<Item key={s._id}>
						<div>
							...
							<div style={{ fontSize: 12, opacity: 0.8 }}>
								Carreras: {(s.careerIds || []).map(id => careersMap.get(id)).filter(Boolean).join(', ') || '—'}
