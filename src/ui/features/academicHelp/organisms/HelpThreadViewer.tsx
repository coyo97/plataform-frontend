...
const HelpThreadViewer: React.FC<{ helpId: string }> = ({ helpId }) => {
	...
	// --- dueño de la ayuda académica ---
	const currentUserId = getUserId();
	const helpUser: any = help.user;
	...
	const helpOwnerId =
		typeof helpUser === 'string'
			...
			const isOwner =
				...
				Boolean(helpOwnerId) &&
				String(helpOwnerId) === String(currentUserId);
			...
			const onDeleteMessageHandler = async (msg: any) => {
				...
				try {
					...
					const userMsg = resolveErrorMessage(err, {
						...
						moderation:
							'No se pudo eliminar el mensaje debido a una restricción del sistema.',
					});
					setErrorMsg(userMsg);
				}
			};
			...
			const onSaveEdit = async () => {
				...
				try {
					...
					const userMsg = resolveErrorMessage(err, {
						...
						moderation:
							'El mensaje fue bloqueado por moderación. Revisa el texto y el archivo adjunto.',
						fileType:
							'Tipo de archivo no permitido para el mensaje. Revisa las extensiones soportadas.',
						fileSize:
							'El archivo que intentas subir es demasiado grande para este mensaje.',
					});
			...
			// wrapper para nuevas respuestas
			const handleSendResponse = async (...args: any[]) => {
				try {
					...
					const userMsg = resolveErrorMessage(err, {
						...
						moderation:
							'La respuesta fue bloqueada por moderación. Revisa que el texto y el archivo adjunto cumplan las políticas.',
						fileType:
							'Tipo de archivo no permitido en las respuestas. Revisa las extensiones soportadas.',
						fileSize:
							'El archivo que intentas adjuntar en la respuesta es demasiado grande.',
					});
			...
			const avatarSrc = help.user?.profile?.profilePicture
				...
				return (
					<SmartBox column gap={3}>
						<ModerationAlert message={errorMsg} onClose={() => setErrorMsg(null)} />
						...
						{help.status === 'resolved' && (
							<Text size="xs" sx={{ color: 'text.secondary', mb: 1 }}>
