...
import { Dialog, DialogContent, IconButton, Button, Box, Menu, MenuItem, ListItemIcon, ListItemText } from '@mui/material';
...
const ProfileDetails: React.FC<Props> = ({ profile, onEditProfile, onChangePhoto }) => {
	...
	const fullName = useMemo(
		() => `${profile.username ?? ''} ${profile.apellidoPaterno ?? ''} ${profile.apellidoMaterno ?? ''}`.trim() || profile.username || '',
		[profile]
	);
	...
	const handleInternalUpload = async (file: File) => {
		try {
			...
			// Si aún no tienes un refetch centralizado, esta es la forma simple:
	...
	return (
		<SmartBox column center radius="md2x" shadow="md" p="px12" sx={{ gap: theme.spacing(3), width: '100%' }}>
			...
			{/* Menú de acciones de foto: Ver / Subir */}
			<Menu
				anchorEl={menuAnchor}
				open={menuOpen}
				onClose={closeMenu}
