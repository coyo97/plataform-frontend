// src/ui/features/publications/organisms/publicationCard/PublicationCard.tsx
...
const PublicationCard: React.FC<Props> = ({
	...
	// ✅ permisos desde backend (normalizados)
	const canEditFromBackend = !!canEditPublication;
	const canDeleteFromBackend = !!canDeletePublication;

	// 🔐 Solo consideramos "dueño con acciones" si:
	//  - es owner
	//  - y tiene AL MENOS un permiso real (editar o eliminar)
	const isOwnerWithAnyPermission =
		isOwner && (canEditFromBackend || canDeleteFromBackend);

	// Estos son los permisos efectivos para botones
	const effectiveCanEdit = isOwnerWithAnyPermission && canEditFromBackend;
	const effectiveCanDelete = isOwnerWithAnyPermission && canDeleteFromBackend;
			...
			<Card
				...
				headerActions={
					<>
						...
						{menuOpen && (
							<ActionMenu
								// ⬇️ AHORA solo somos "dueño" si también tenemos permisos del backend
								isOwner={isOwnerWithAnyPermission}
