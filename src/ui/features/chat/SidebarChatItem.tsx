// src/ui/features/chat/SidebarChatItem.tsx
...
interface SidebarChatItemProps {
	...
	onClick: () => void;
	subtitle?: string;
	badge?: number;      // número de mensajes no leídos
...
export const SidebarChatItem: React.FC<SidebarChatItemProps> = ({
	...
	const imgUrl = isAbsolute
		? rel
		: rel
			? (rel.startsWith('/') ? `${HOST}${rel}` : `${HOST}/${rel}`).replace(
					/([^:]\/)\/+/g,
					'$1'
			  )
		...
		<ChatListItem isActive={isActive} onClick={onClick}>
			...
			>
				...
				>
					...
					{/* Dot de presencia superpuesto (abajo-derecha) */}
					<span
						style={{
							position: 'absolute',
							right: -1,
					...
					{/* Badge de no leídos (arriba-derecha) */}
					{showBadge && (
						<span
							style={{
								position: 'absolute',
