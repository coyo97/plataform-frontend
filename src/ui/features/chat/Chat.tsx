...
import { sendMessage, sendFileMessage, deleteMessage as delMsg, fetchConversations,
	...
	const handleSend = async (content: string, file?: File | null) => {
		...
		if (file) {
			...
			try {
				...
				if (!isGroupMessage) {
					// DM: lo pintamos como enviado
					setMessages(prev => [...prev, { ...saved, status: 'sent' } as any]);
					bumpConversation(currentChatId, content || '📎 Archivo', saved.createdAt, false);
				}
				// En grupos NO añadimos optimista para evitar duplicados:
				// esperas el evento 'receive-message' del socket (que ya te llega bien).
			} catch (e) {
		...
		// --------- TEXTO ----------
		if (!isGroupMessage) {
			// DM con optimista + ACK (evita incertidumbre)
			...
			// Emitimos con ACK (callback). NO usamos el wrapper sendMessage para poder recibir el ACK.
			socket.emit(
				'send-message',
				{
					senderId: userId,
					...
					content,
					isGroupMessage: false,
					clientId, // importante para reconciliar en el frontend y opcionalmente en backend
				},
				(ack?: { ok: boolean; message?: any; clientId?: string }) => {
					if (ack && ack.ok && ack.message) {
						// Reemplazamos el temporal por el definitivo y marcamos 'sent'
						setMessages(prev =>
									prev.map(m =>
								   ...
								   bumpConversation(currentChatId, content, ack.message.createdAt, false);
					} else {
						// Marcamos como fallido
						setMessages(prev =>
									prev.map(m =>
											 (m as any).clientId === clientId ? { ...m, status: 'failed' } : m
											)
								   );
					}
				}
			...
			setTimeout(() => {
				setMessages(prev =>
							prev.map(m =>
									 (m as any).clientId === clientId && (m as any).status === 'sending'
										 ? { ...m, status: 'failed' }
										 : m
									)
						   );
			...
			// GRUPO: sin optimista (así evitas ver doble). Solo emitimos
			...
			// Esperas a 'receive-message' para pintar (ya funciona bien en tu app).
		}
	};
